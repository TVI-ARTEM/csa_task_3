...
"""ltriangle.py - содержит описание нижнетреугольной квадратной матрицы."""

"""Нижнетреугольная квадратная матрица."""


class LTriangle(Matrix):
    ...
    def initialize(self, args: []):
        if len(args) != self.dimension * (self.dimension + 1) / 2:
            self.correct = False
            raise ValueError("Incorrect length of arguments.")

        self.numbers.clear()

        for num_index in range(int(self.dimension * (self.dimension + 1) / 2)):
    ...
    def __str__(self):
        result = "It's low triangle matrix. Dimension = {0}\n".format(self.dimension)
        if self.correct:
            current_index = 0
